...
export interface LonghornStorage {
  maximum: number;
  available: number;
  reserved: number;
  scheduled: number;
...
export interface LonghornEvent {
  reason: string;
  count: number;
  lastSeen: string;
  age: string;
...
export interface LonghornOverview {
  volumeApiVersion?: string | null;
  nodeApiVersion?: string | null;
  volume: OverviewCard;
  node: OverviewCard;
...
export type PanelTab = "detail" | "data" | "yaml" | "logs" | "exec" | "diff" | "portforward";
