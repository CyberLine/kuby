...
pub async fn start_watch(
    ...
    if !cache.is_listable(&api_version, &kind) {
    ...
    if !cache.is_watchable(&api_version, &kind) {
        return Err(KubyError::Message(format!(
            "{kind} does not support watch (list-only API)"
        )));
    }
        ...
        while let Some(item) = stream.next().await {
            match item {
                ...
                Err(err) => {
                    ...
                    // Transient stream drops (idle timeouts, LB resets, etc.) are normal;
                    // kube-runtime reconnects with backoff. Only surface hard failures in the UI.
                    if unrecoverable {
                        let _ = app2.emit(
                            "k8s://watch-error",
                            WatchErrorPayload {
